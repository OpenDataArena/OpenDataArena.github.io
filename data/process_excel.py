...
def parse_excel_advanced():
    try:        
        ...
        df_dataset = pd.read_excel('0726.xlsx', sheet_name='dataset', header=None)
        dataset_attr_map = {}
        ...
        for sheet_name in sheets_to_process:
            ...
            df_raw = pd.read_excel('0726.xlsx', sheet_name=sheet_name, header=None)
            ...
            if instruct_row is not None:
                ...
                if sheet_name == 'llama':
                    instruct_name = 'Llama-3.1-8B-Instruct'
                elif sheet_name == 'qwen':
            ...
            # 在插入base和instruct模型之前，为它们分配正确的ID
            if instruct_info:
                instruct_info['id'] = 0  # instruct模型使用ID 0
            if base_info:
                base_info['id'] = 1  # base模型使用ID 1
...
def detect_column_layout(df_raw):
    ...
    default_config = {
        ...
        'math_cols': list(range(7, 13)), 
        'code_cols': list(range(13, 17)),       
        'reasoning_cols': [17, 18, 19, 20, 21] 
    }
